#ifndef RK_calculations_hpp
#define RK_calculations_hpp
...
public:
    ...
    void fill_parameters( long double step, int option_ch/*, long double scale_s*/)//uzupelnianie parametrow
    {
        option = option_ch;
        h = step;
        //scale = scale_s; <- nie uzywamy skali w trybie bez rysowania
    }
    ...
    long double f1(long double x, long double y1, long double y2, long double y3, long double y4) //pierwsze rownanie w ukladzie
    {
        ...
        case 4:
            return (-(0.839*y1 + 0.5*(-0.419-0.839)*(std::abs(y1+1)-std::abs(y1-1))) + y3)*(1/1.02);
        case 5:
            return y3;
        case 6:
